...
class Log_Content {
  ...
  // @@protoc_insertion_point(class_scope:src.Content)
}
...
class Log {
  ...
  function read($fp, &$limit = PHP_INT_MAX) {
    while(!feof($fp) && $limit > 0) {
      ...
      $wire  = $tag & 0x07;
      $field = $tag >> 3;
      //var_dump("src: Found $field type " . Protobuf::get_wiretype($wire) . " $limit bytes left");
      switch($field) {
        case 1:
          ASSERT('$wire == 0');
  ...
  // repeated .src.Content Contents = 2;
  ...
  // @@protoc_insertion_point(class_scope:src)
}
